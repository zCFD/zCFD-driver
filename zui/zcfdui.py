...
from zui.utils import _
#from spyderlib.config import CONF
#from spyderlib.guiconfig import (CUSTOM_COLOR_SCHEME_NAME,
#                                 set_default_color_scheme)
#
from zui.utils import (get_icon)
from zui.utils import NoDefault
...
class ConfiguratorWindow(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle(_("zCFD Configurator"))
...
class SpyderConfigPage(ConfigPage, ConfigAccessMixin):
    ...
    def create_double_spinbox(self, prefix, suffix, option, default=NoDefault,
                       min_=None, max_=None, step=None, tip=None):
        if prefix:
            plabel = QLabel(prefix)
        else:
...
class MainConfigPage(GeneralConfigPage):
    ...
    def setup_page(self):
        ...
        choices = [('SI',1)]
        units, box = self.create_combobox(_('Units'), choices,
                                           'units')#,
                                           #default=self.main.default_style)
        
        choices = [('IC_1',1)]
        reference, box = self.create_combobox(_('Reference State'), choices,
                                           'reference_state')#,
                                           #default=self.main.default_style)
        choices = [('scotch',1),('metis',2)]
        partitioner, box = self.create_combobox(_('Partitioner'), choices,
                                           'partitioner')#,
                                           #default=self.main.default_style)
        
        restart_box = newcb(_("Restart from previous solution"),
                                    'restart',
        ...
        general_layout.addWidget(units)
        general_layout.addWidget(reference)
        general_layout.addWidget(partitioner)
        general_layout.addWidget(restart_box)
        general_group.setLayout(general_layout)
        
        time_marcher_group = QGroupBox(_("Time Marcher"))
        
        multigrid = self.create_spinbox("Multigrid", "meshes", 'multigrid',4,
                                          1, 10, 1)
        cfl_spin = self.create_double_spinbox("CFL", "", 'cfl',2.0,
                                          0.5, 2.5, 0.1)
        cfl_spin.setEnabled(True)
        cycles_spin = self.create_spinbox("Cycles", "", 'cycle',1000,
                                          0, 20000, 500,
                                          tip=_("Number of cycles"))
        cycles_spin.setEnabled(True)
        
        unsteady_group = QGroupBox(_("Time accurate"))
        
        time_marcher_layout = QVBoxLayout()
        time_marcher_layout.addWidget(multigrid)
        time_marcher_layout.addWidget(cfl_spin)
        time_marcher_layout.addWidget(cycles_spin)
        
        time_marcher_group.setLayout(time_marcher_layout)
        ...
        vlayout.addWidget(time_marcher_group)
...
class EqnConfigPage(GeneralConfigPage):
    CONF_SECTION = "main"
    ...
    def setup_page(self):
        ...
        eqn_group =  QGroupBox(_("Equations"))

        eqn_layout = QVBoxLayout()
        choices = [('euler',1),('RANS',2),('viscous',3)]
        equation, self.box = self.create_combobox(_('Equations'), choices,
                                           'equations')#,
                                           #default=self.main.default_style)
        self.connect(self.box,SIGNAL("currentIndexChanged(const QString&)"),self.eqn_changed)
        eqn_layout.addWidget(equation)
        eqn_group.setLayout(eqn_layout)


        self.euler_group = QGroupBox(_("Euler Equations"))
        euler_layout = QVBoxLayout()
        self.euler_group.setLayout(euler_layout)

        self.rans_group = QGroupBox(_("RANS Equations"))
        self.rans_layout = QVBoxLayout()
        self.rans_group.setLayout(self.rans_layout)
        self.rans_group.setVisible(False)
        
        self.vlayout = QVBoxLayout()
        self.vlayout.addWidget(eqn_group)
        self.vlayout.addWidget(self.euler_group)
        self.vlayout.addWidget(self.rans_group)
        self.vlayout.addStretch(1)
        self.setLayout(self.vlayout)
        
    def eqn_changed(self):
        
        index = self.box.currentIndex()
        key = self.box.itemData(index)
        self.euler_group.setVisible(False)
        self.rans_group.setVisible(False)
        if key == 1:
            self.euler_group.setVisible(True)
        elif key == 2:
            self.rans_group.setVisible(True)
...
def main():
    ...
    mainpage = MainConfigPage(frame,frame)
    mainpage.initialize()
    frame.add_page(mainpage)
    eqnpage = EqnConfigPage(frame,frame)
    eqnpage.initialize()
    frame.add_page(eqnpage)
